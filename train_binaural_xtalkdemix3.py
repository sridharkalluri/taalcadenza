...
writer = SummaryWriter('runs/xtalkdemix3_2')
...
def train(dataloader, model, loss_fn, optimizer, epoch):
    ...
    model.train()
    for batch, (X, xin, _, y) in enumerate(dataloader):
        print(f"Batch # {batch}")
        #if device == 'mps':
...
dataset = AtMicCadenzaICASSP2024_3(
...
embed_dataloader = DataLoader(dataset, batch_size=512, shuffle=True)
...
if 0:
    ...
    train_features, train_input, train_spat, train_target = next(iter(train_dataloader))
    #writer.add_graph(model, train_features.to(torch.float32).to(device))

    test_features, test_input, _, test_target = next(iter(test_dataloader))
...
for t in range(EPOCHS):
    ...
    torch.save(model.state_dict(),f"xtalkdemix3_model_weights_epoch{t+1}.pth")
